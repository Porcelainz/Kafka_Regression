...
object Experiment3_3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(q1 + tree.hen(generateID(q1)).state)
    println(q2 + tree.hen(generateID(q2)).state)
    println(q3 + tree.hen(generateID(q3)).state)
    ...
    val inputValues3 = Array("p1:9")
    println("------ Input Value = 9 ------")
