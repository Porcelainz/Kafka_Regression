...
class NotificationNode1(_name: String, _url: String = "") extends Serializable {
  var name = _name
  var parent: NotificationNode1 = _ 
  ...
  def setParent(parent: NotificationNode1): Unit = {
    this.parent = parent
  }
...
class StreamingNode1(appName: String, topic: String) extends Serializable{
  //var counter = 0
  // val node_1 = node1
  // val node_2 = node2
  ...
  val child1 = new NotificationNode1("child1", "")
  val child2 = new NotificationNode1("child2", "") 
  val parent = new NotificationNode1("parent", "https://maker.ifttt.com/trigger/scala_event/json/with/key/cyMr3y7V3Np-gzMAhWE8HM")
    ...
    if (!batchDF.isEmpty) {
      batchDF.foreach(x =>
        x(0).toString().toDouble match {
          case d:Double if (d > 5.0) =>  child1.changeResult(true)
          case d:Double if (d < 3.0) =>  child2.changeResult(true)
